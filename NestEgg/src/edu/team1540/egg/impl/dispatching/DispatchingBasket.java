...
public class DispatchingBasket<S extends Schema> extends FragmentBasket {
	...
	public DispatchingBasket(final Address address, final ConnectionHandler h, final S start, final BasketDrawCallbacks a, final String title, final DispatchingFragment<S>... fragments) {
		super(a, title, fragments);
		final AtomicReference<S> currentSchema = new AtomicReference<S>(start);
		final AtomicCallback<ConnectedThread> callback = new AtomicCallback<ConnectedThread>();
